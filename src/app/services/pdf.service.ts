...
import {
  ...
  NavigationSide,
  IViewGroup
} from "../models/view-group.model.js";
...
export class PdfService {
  ...
  private activePages = [];
  private pages: BehaviorSubject<IPdfPage[]> = new BehaviorSubject(
    this.activePages
  );
  ...
  async initLastPage() {
    ...
    if (!!val) {
      const lastPage =
        this.activePages.find(p => p.pageNumber === Number(val)) ||
  ...
  async initViewGroupAndPages() {
    ...
    if (!!viewGroup) {
      ...
      viewGroup = {
        name: ViewGroupName.arapca_meal,
  ...
  private updateActivePages(viewGroupName: ViewGroupName) {
    this.activePages =
      viewGroupName === ViewGroupName.arapca_meal
        ? [...pdfInfo.pages]
        : pdfInfo.pages
            .filter(p => p.group === viewGroupName)
            .map((p, index) => ({ ...p, pageIndex: index }));
  ...
  scaleZoomFactor(scale: number) {
    let newFactor =
      this.zoomFactor.value + this.zoomFactor.value * (scale - 1) * 0.2;
  ...
  async setViewGroup(viewGroupName: ViewGroupName) {
    ...
    const viewGroups =
      ((await this.storage.get("ViewGroups")) as IViewGroup[]) || [];
    ...
    viewGroup =
      index !== -1
        ...
        : {
            name: viewGroupName,
            navSide: this.getDefaultNavSide(viewGroupName)
          };
